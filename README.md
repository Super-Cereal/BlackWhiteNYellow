...
3. Добавить `.env` файл в корень папки с nodejs сервером с единственной строкой
   ...
   authToken="uuiiayuer"
   ```
   Где uuiiayuer - токен авторизации для [бекенда Яндекса](https://shri.yandex/hw/api/index.html)(Без "Bearer")
4. `npm start` для репозитория с nodejs сервером
5. Перейти на `localhost:3000`

## Redux

Redux store, селекторы, мидлвары, и редьюсер для App компоненты лежать в корневой redux папке.
...
Цвета, подключение шрифтов, sass-mixin-ы для адаптивной верстки и некоторые глобальные стили хранятся в папке [/style/sass/]
